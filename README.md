...
- Mac
  - [Impactor](https://github.com/claration/Impactor/releases/latest/download/Impactor-macos-universal.dmg)
- Windows
  - [iTunes](https://apple.co/ms)
  - Impactor [32位元](https://github.com/claration/Impactor/releases/latest/download/Impactor-windows-x86-portable.exe) / [64位元](https://github.com/claration/Impactor/releases/latest/download/Impactor-windows-x86_64-portable.exe) 依自己電腦選擇
- iOS
  - 安裝 [LocalDevVPN](https://apps.apple.com/us/app/localdevvpn/id6755608044)，用於 iOS 開發環境下的 VPN / 通訊設定
  - 請務必先開啟 `開發者模式`
  - iOS 務必連接上電腦，並取得信任
  - 下載最新的 [UFOGeo IPA](https://github.com/Leoohyeah/UFOGeo/releases/latest/download/UFOGeo.ipa)

### 2. 安裝 LiveContainer + SideStore

1. 下載 [LiveContainer+SideStore.ipa](https://github.com/LiveContainer/LiveContainer/releases/download/nightly/LiveContainer+SideStore.ipa)
2. 電腦安裝完對應的 Impactor
3. 打開 Impactor
4. 點擊右上角齒輪 → 新增帳號登入 Apple ID → 登入後回前頁
5. 下方點擊 `匯入 .ipa` → 選擇 `LiveContainer+SideStore.ipa`
6. 安裝完成後下方點擊 `工具` → 開啟 `使用遠端配對` → 放置配對檔案

### 3. 在 iOS 執行 LiveContainer App

1. iOS 前往「設定 → 一般 → VPN 與裝置管理」，信任用來簽署開發者憑證
2. iOS 開啟 LocalDevVPN 點選連線 
3. iOS 開啟 LiveContainer
4. 點擊下方 App → 左上角 `+` 右邊圖示 → 會進到另一個 SideStore App
5. 點擊下方 Settings → 登入 Apple ID → 關閉 App
6. 重新開啟 LiveContainer → 點擊右下角 設定 → 從 SideStore 載入證書
7. 點選 App → 左上角 `+` → 選擇下載的 UFOGeo IPA

### 4. 在 LiveContainer 執行 UFOGeo

1. 執行 UFOGeo，在匯入配對檔案的位置選擇 → 右下角瀏覽 → LiveContainer → SideStore → Documents → 點選 `ALTPairingFile.mobiledevicepairing`
...
- [Impactor](https://github.com/claration/Impactor)：提供 macOS / Windows 環境下的配對與裝置連線工具支援
- [LiveContainer](https://github.com/LiveContainer/LiveContainer)：提供 Healthkit 啟用權限
...
使用者可在 App 的「帳號與訂閱」內刪除會員。系統會先停止可續訂方案，再刪除 Firebase Authentication 帳號與會員資料；依法令、稅務、會計、交易爭議、防詐及資訊安全所需的付款或稽核紀錄，可能在必要期間內保留。完整內容請參閱 [UFOGeo 隱私權政策](https://leoohyeah.github.io/UFOGeo/privacy/)。
